...
plotDataTable2 = array2table(epsilon','VariableNames',{'epsilon'});

%usematrixでforを回すため
for mi = 1:length(struct2cell(usematrix))
    for i = 1:epsiloncol
        Rsec(i) = calcevatest(usematrix(mi).matrix,deltadeta(mi).delta,epsilon(i));
    end
    sym(Rsec)
%     T = array2table(evalution,'VariableNames',{usematrix(mi).name});
%     plotDataTable(:,mi+1) = T 
%データテーブルに行列の名前の列を作成し、そこにRsecを代入
 plotDataTable2(:,{usematrix(mi).name}) = array2table(Rsec) ;
end
